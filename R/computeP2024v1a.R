...
#' @param discretizations number of discretizations (matters only for \code{dimensions=discretizations=1})
#' @param contrast.mask boolean mask on \code{IG} specifying which variables are contrast variables (or \code{NULL} if none, otherwise at least 3 variables must be marked). \code{NULL} is allowed only for \code{dimensions=discretizations=1}.
...
# medians are more robust, lack of the factor log(2) gives a linear growth
  ...
  if (param.median) {
   ...
   #This is the exact distribution but fails for very small values
   ...
   p.value <- -expm1(-chisq.log/M*(digamma(n.important)-digamma(n.important/2)))
