...
#' @examples
#' \donttest{
#' discr_data<- as.data.frame(lapply(as.data.frame(madelon$data[,1:3]), 
#' 				function(column) (column > median(column) )*1. ))
#' pack.confounders(discr_data, all.combinations=TRUE)
#' }
#' @export 
...
#' @examples
#' \donttest{
#' Confounders<- as.data.frame(lapply(as.data.frame(madelon$data[,1:3]), 
#' function(column) (column > median(column) )*1. ))
#' MDFS.confounded(madelon$data[,-c(1:3)], madelon$decision, confounders=Confounders, 
#' dimensions = 2, divisions = 1,
#'      range = 0, seed = 0)
#' }
...
#' @examples
#' \donttest{
#' discr_data<- as.data.frame(lapply(as.data.frame(madelon$data), 
#' function(column) (column > median(column) )*1. ))
#' Confounders<- discr_data[,1:3]
#' MDFS.discrete.confounded(discr_data[,-c(1:3)], madelon$decision,
#' confounders=Confounders, dimensions = 2,seed=0)
#' }
#' @importFrom stats p.adjust
#' @export
...
#' @examples
#' \donttest{
#' data_bin<-as.data.frame(lapply(as.data.frame(madelon$data),
#'  function(x) 1.*(x>median(x)) ))
#' find_fully_explained_vars_idx(data_bin, data_bin[,2])
#' }
#' @export
...
#' discr_data<- as.data.frame(lapply(as.data.frame(madelon$data),
#'  function(column) (column > median(column) )*1. ))
#' confounder<- 1.*(madelon$data[,1] > median(madelon$data[,1]))
#' pairwiseMIsimilarityDiscrete.confounded(discr_data[,-1], confounder)
#' }
#' @importFrom stats p.adjust
#' @export
...
#' \donttest{
#' confounder<- 1.*(madelon$data[,1] > median(madelon$data[,1]))
#' pairwiseMIsimilarity.confounded(madelon$data[,-1],confounder)
#' }
#' @importFrom stats p.adjust
#' @export
